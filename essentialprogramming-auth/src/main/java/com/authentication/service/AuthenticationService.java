...
import com.authentication.identityprovider.internal.entities.AccountPlatform;
...
public class AuthenticationService {
    ...
    private AccessToken loginWithPassword(AuthRequest authRequest, Language language) throws ApiException {
        ...
        AccountPlatform accountPlatform = account.getPlatforms().stream().filter(accountPlatform1 -> accountPlatform1
                .getPlatformType().toString().contentEquals(authRequest.getPlatform().toString())).findFirst().orElseThrow(
                () -> new ApiException(Messages.get("USER.NOT.EXIST", language), HTTPCustomStatus.UNAUTHORIZED)
        );

        Map<String, String> privateClaimMap =
                privateClaims(account.getEmail(), account.isActive(), accountPlatform.getRoles());

        LocalDateTime now = LocalDateTime.now();
        LocalDateTime expirationTime = now.plus(1, ChronoUnit.DAYS);

        return getAccessToken(now, expirationTime, privateClaimMap, account.isActive(), account.getEmail());
    }
    ...
    private AccessToken getAccessToken(LocalDateTime now, LocalDateTime expirationTime, Map<String, String> privateClaimMap, boolean active, String email) {
        long expiresIn = ChronoUnit.MINUTES.between(now, expirationTime);
        return AccessToken.builder()
                .accessToken(tokenService.generateJwtToken(expiresIn, privateClaimMap, Collections.emptyMap()))
